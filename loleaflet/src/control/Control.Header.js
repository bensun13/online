...
L.Control.Header = L.Control.extend({
	...
	canvasDPIScale: function () {
		...
		var scale = docLayer && docLayer.canvasDPIScale ? docLayer.canvasDPIScale() : L.getDpiScaleFactor(true);
		return scale;
	},
...
L.Control.Header.HeaderInfo = L.Class.extend({
	...
	update: function () {
		...
		var endPx = this._isCol ? bounds.getBottomRight().x / multiplier: bounds.getBottomRight().y / multiplier;
		...
		if (splitPosContext) {
			...
			if (splitIndex) {
				...
				splitPos = this._dimGeom.getElementData(splitIndex).startpos / multiplier;
